<script>
    ...
    const getPaginatedData = () => {
        ...
        paginatedData = workoutData.slice(startIndex, endIndex);
    }

    const nextPage = () => {
        if(workoutData.length/7 > currentPage ) {
            currentPage++;
            getPaginatedData();
        }
    }
    ...
    const handleDelete = async (ID) => {
        try {
            const response = await fetch(`http://localhost:8000/api/workout/${ID}`, {
                method: 'DELETE',
                headers: {
...
<div class="m-5 ">
    <h1 class="text-4xl font-bold text-cyan-700 pt-5">Workout List</h1>
    <div class="flex justify-end">
        <a href="/workout/add">
            <button class="rounded-lg bg-cyan-500 text-white font-bold p-2 my-5 hover:bg-sky-600 ...">+ Add Workout</button>
        </a>
    </div>
    ...
    <div class="p-4 bg-cyan-200 rounded-lg">
        ...
        <!-- Table Header -->
            ...
            <!-- Table Rows -->
            <!-- {#each array as value (value)} -->
            {#each paginatedData as value (value._id)} <!-- Assuming _id is the unique identifier in your data -->
...
{#if $isModalOpen}
  <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white p-6 rounded-lg shadow-md">
      ...
      <h1>{deleteWorkout.name}</h1>
      <h1>{deleteWorkout.description}</h1>
      <div class="mt-4 flex justify-end gap-1">
        ...
        <button class="rounded-lg bg-red-500 text-white font-bold p-2 my-5 hover:bg-red-600 ..." on:click={() => handleDelete(deleteWorkout._id)}>Delete</button>
      </div>
    </div>
  </div>
{/if}
